...
/**
 * Helper function for validing the bump release type.
...
/**
 * Helper function for validing the json indent value.
 * @param {Object} opts - The options for this plugin.
 */
const validateJsonIndent = (opts) => {
    ...
    if (indent !== opts.indent) {
        opts.indent = defaultJsonIndent;
    } else if (indent < 1 || indent > 10) {
        opts.indent = defaultJsonIndent;
    }
};

/**
 * Helper function for validing the types of the version property values.
 * @param {Object} opts - The options for this plugin.
 */
...
/**
 * Helper function for validing the plugin options.
 * @param {Object} opts - The options for this plugin.
 *
...
const validateOptions = (opts) => {
    ...
    validateJsonIndent(opts);
    validateVersionPropertyType(opts);
...
const getTaskVersion = (task) => {
    ...
    if (!major || !minor || !patch) {
        throw new Error('Invalid task. Task must represent version as an object under the \'version\' key' +
...
module.exports = {
    ...
    validateReleaseType: validateReleaseType,
    validateJsonIndent: validateJsonIndent,
    validateVersionPropertyType: validateVersionPropertyType,
    getTaskVersion: getTaskVersion,
